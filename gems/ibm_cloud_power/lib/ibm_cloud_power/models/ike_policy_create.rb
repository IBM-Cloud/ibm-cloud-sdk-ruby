...
module IbmCloudPower
  ...
  class IKEPolicyCreate < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::IKEPolicyCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] preshared_key Value to be assigned
    def preshared_key=(preshared_key)
      if preshared_key.nil?
        fail ArgumentError, 'preshared_key cannot be nil'
      end
