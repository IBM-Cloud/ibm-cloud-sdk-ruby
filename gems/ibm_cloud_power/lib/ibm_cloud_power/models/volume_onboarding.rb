...
module IbmCloudPower
  class VolumeOnboarding < ApiModelBase
    ...
    def self.openapi_types
      {
        :'description' => :'String',
        :'id' => :'String',
        :'input_volumes' => :'Array<String>',
        :'status' => :'String',
        :'creation_timestamp' => :'Time',
        :'progress' => :'Float',
        :'results' => :'VolumeOnboardingAllOfResults'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::VolumeOnboarding` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::VolumeOnboarding`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'input_volumes')
        if (value = attributes[:'input_volumes']).is_a?(Array)
          self.input_volumes = value
        end
      end
    ...
    def hash
      [description, id, input_volumes, status, creation_timestamp, progress, results].hash
    end
