  ...
  class Operations < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::Operations` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::Operations`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'boot_mode')
        self.boot_mode = attributes[:'boot_mode']
      end

      if attributes.key?(:'operating_mode')
        self.operating_mode = attributes[:'operating_mode']
      end
    ...
    def valid?
      ...
      boot_mode_validator = EnumAttributeValidator.new('String', ["a", "b", "c", "d"])
      return false unless boot_mode_validator.valid?(@boot_mode)
      operating_mode_validator = EnumAttributeValidator.new('String', ["normal", "manual"])
      return false unless operating_mode_validator.valid?(@operating_mode)
      task_validator = EnumAttributeValidator.new('String', ["consoleservice", "dston", "dumprestart", "iopdump", "iopreset", "remotedstoff", "remotedston", "retrydump"])
      return false unless task_validator.valid?(@task)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] boot_mode Object to be assigned
    def boot_mode=(boot_mode)
      validator = EnumAttributeValidator.new('String', ["a", "b", "c", "d"])
      unless validator.valid?(boot_mode)
        fail ArgumentError, "invalid value for \"boot_mode\", must be one of #{validator.allowable_values}."
      end
      @boot_mode = boot_mode
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] operating_mode Object to be assigned
    def operating_mode=(operating_mode)
      validator = EnumAttributeValidator.new('String', ["normal", "manual"])
      unless validator.valid?(operating_mode)
        fail ArgumentError, "invalid value for \"operating_mode\", must be one of #{validator.allowable_values}."
      end
    ...
    def task=(task)
      validator = EnumAttributeValidator.new('String', ["consoleservice", "dston", "dumprestart", "iopdump", "iopreset", "remotedstoff", "remotedston", "retrydump"])
    ...
    def hash
      [boot_mode, operating_mode, task].hash
    end
