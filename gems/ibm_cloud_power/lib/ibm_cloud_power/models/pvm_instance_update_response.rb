...
module IbmCloudPower
  class PVMInstanceUpdateResponse < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::PVMInstanceUpdateResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [allow_remote_restart, default_trusted_profile, license_repository_capacity, memory, metadata_service, pin_policy, preferred_processor_compatibility_mode, proc_type, processors, server_name, status_url, storage_pool_affinity, virtual_cores].hash
    end
