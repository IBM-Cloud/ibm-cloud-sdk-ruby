...
module IbmCloudPower
  class VolumeGroupStorageDetails < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::VolumeGroupStorageDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'consistency_group_name')
        ...
        self.consistency_group_name = nil
      end
    ...
    # @param [Object] consistency_group_name Value to be assigned
    def consistency_group_name=(consistency_group_name)
      if consistency_group_name.nil?
        fail ArgumentError, 'consistency_group_name cannot be nil'
      end
