...
module IbmCloudPower
  class WorkspaceSSHKey < ApiModelBase
    ...
    attr_accessor :primary_workspace
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::WorkspaceSSHKey` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::WorkspaceSSHKey`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'primary_workspace')
        self.primary_workspace = attributes[:'primary_workspace']
      else
        self.primary_workspace = nil
      end
    ...
    def list_invalid_properties
      ...
      if @primary_workspace.nil?
        invalid_properties.push('invalid value for "primary_workspace", primary_workspace cannot be nil.')
      end

      if @ssh_key.nil?
        invalid_properties.push('invalid value for "ssh_key", ssh_key cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @primary_workspace.nil?
      return false if @ssh_key.nil?
      visibility_validator = EnumAttributeValidator.new('String', ["account", "workspace"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] primary_workspace Value to be assigned
    def primary_workspace=(primary_workspace)
      if primary_workspace.nil?
        fail ArgumentError, 'primary_workspace cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] ssh_key Value to be assigned
    def ssh_key=(ssh_key)
      if ssh_key.nil?
        fail ArgumentError, 'ssh_key cannot be nil'
      end
    ...
    def visibility=(visibility)
      validator = EnumAttributeValidator.new('String', ["account", "workspace"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          id == o.id &&
          name == o.name &&
          primary_workspace == o.primary_workspace &&
          ssh_key == o.ssh_key &&
    ...
    def hash
      [creation_date, description, id, name, primary_workspace, ssh_key, visibility].hash
    end
