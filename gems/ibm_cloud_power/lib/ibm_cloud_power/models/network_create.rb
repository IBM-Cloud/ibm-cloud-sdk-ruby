...
module IbmCloudPower
  class NetworkCreate < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::NetworkCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'advertise')
        ...
        self.advertise = 'enable'
      end

      if attributes.key?(:'arp_broadcast')
        self.arp_broadcast = attributes[:'arp_broadcast']
      else
        self.arp_broadcast = 'disable'
      end
    ...
    def valid?
      ...
      arp_broadcast_validator = EnumAttributeValidator.new('String', ["enable", "disable"])
      return false unless arp_broadcast_validator.valid?(@arp_broadcast)
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] arp_broadcast Object to be assigned
    def arp_broadcast=(arp_broadcast)
      validator = EnumAttributeValidator.new('String', ["enable", "disable"])
      unless validator.valid?(arp_broadcast)
        fail ArgumentError, "invalid value for \"arp_broadcast\", must be one of #{validator.allowable_values}."
      end
      @arp_broadcast = arp_broadcast
    end
    ...
    def hash
      [access_config, advertise, arp_broadcast, cidr, dns_servers, gateway, ip_address_ranges, jumbo, mtu, name, type, user_tags].hash
    end
