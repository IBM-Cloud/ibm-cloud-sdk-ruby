...
module IbmCloudPower
  class SharedProcessorPoolDetailSharedProcessorPool < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::SharedProcessorPoolDetailSharedProcessorPool` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::SharedProcessorPoolDetailSharedProcessorPool`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [allocated_cores, available_cores, creation_date, crn, dedicated_host_id, host_group, host_id, id, name, reserved_cores, shared_processor_pool_placement_groups, status, status_detail, user_tags].hash
    end
