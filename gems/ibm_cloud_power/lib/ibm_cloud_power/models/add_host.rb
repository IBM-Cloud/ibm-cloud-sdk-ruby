  ...
  class AddHost < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::AddHost` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::AddHost`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @display_name.to_s.length > 32
        invalid_properties.push('invalid value for "display_name", the character length must be smaller than or equal to 32.')
      end

      if @display_name.to_s.length < 2
        invalid_properties.push('invalid value for "display_name", the character length must be greater than or equal to 2.')
      end

      pattern = Regexp.new(/^[A-Za-z0-9](?:[A-Za-z0-9_-]{0,30}[A-Za-z0-9])?$/)
      if @display_name !~ pattern
        invalid_properties.push("invalid value for \"display_name\", must conform to the pattern #{pattern}.")
      end

      if @sys_type.nil?
        invalid_properties.push('invalid value for "sys_type", sys_type cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @display_name.to_s.length > 32
      return false if @display_name.to_s.length < 2
      return false if @display_name !~ Regexp.new(/^[A-Za-z0-9](?:[A-Za-z0-9_-]{0,30}[A-Za-z0-9])?$/)
      return false if @sys_type.nil?
    ...
    def display_name=(display_name)
      ...
      if display_name.to_s.length > 32
        fail ArgumentError, 'invalid value for "display_name", the character length must be smaller than or equal to 32.'
      end

      if display_name.to_s.length < 2
        fail ArgumentError, 'invalid value for "display_name", the character length must be greater than or equal to 2.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] sys_type Value to be assigned
    def sys_type=(sys_type)
      if sys_type.nil?
        fail ArgumentError, 'sys_type cannot be nil'
      end
    ...
    def hash
      [display_name, sys_type, user_tags].hash
    end
