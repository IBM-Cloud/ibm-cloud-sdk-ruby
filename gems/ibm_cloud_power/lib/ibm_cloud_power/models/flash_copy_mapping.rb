  ...
  class FlashCopyMapping < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::FlashCopyMapping` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::FlashCopyMapping`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'copy_rate')
        self.copy_rate = attributes[:'copy_rate']
      end

      if attributes.key?(:'flash_copy_name')
        self.flash_copy_name = attributes[:'flash_copy_name']
      else
        self.flash_copy_name = nil
      end
      ...
      if attributes.key?(:'source_volume_name')
        self.source_volume_name = attributes[:'source_volume_name']
      end
      ...
      if attributes.key?(:'target_volume_name')
        self.target_volume_name = attributes[:'target_volume_name']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @flash_copy_name.nil?
        invalid_properties.push('invalid value for "flash_copy_name", flash_copy_name cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @flash_copy_name.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] flash_copy_name Value to be assigned
    def flash_copy_name=(flash_copy_name)
      if flash_copy_name.nil?
        fail ArgumentError, 'flash_copy_name cannot be nil'
      end

      @flash_copy_name = flash_copy_name
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          copy_rate == o.copy_rate &&
          flash_copy_name == o.flash_copy_name &&
          progress == o.progress &&
          source_volume_name == o.source_volume_name &&
          start_time == o.start_time &&
          status == o.status &&
          target_volume_name == o.target_volume_name
    end
    ...
    def hash
      [copy_rate, flash_copy_name, progress, source_volume_name, start_time, status, target_volume_name].hash
    end
