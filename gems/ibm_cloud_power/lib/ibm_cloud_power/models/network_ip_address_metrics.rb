...
module IbmCloudPower
  # IP Address Metrics
  class NetworkIpAddressMetrics < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::NetworkIpAddressMetrics` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::NetworkIpAddressMetrics`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] utilization Value to be assigned
    def utilization=(utilization)
      if utilization.nil?
        fail ArgumentError, 'utilization cannot be nil'
      end
    ...
    def hash
      [available, total, used, utilization].hash
    end
