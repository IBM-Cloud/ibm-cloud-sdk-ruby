...
module IbmCloudPower
  ...
  class StoragePoolCapacity < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::StoragePoolCapacity` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::StoragePoolCapacity`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'storage_host')
        self.storage_host = attributes[:'storage_host']
      end
    ...
    def hash
      [available_capacity, max_allocation_size, pool_name, replication_enabled, storage_host, storage_type, total_capacity].hash
    end
