...
module IbmCloudPower
  class CloudConnectionVirtualPrivateCloudsVirtualPrivateCloudsInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::CloudConnectionVirtualPrivateCloudsVirtualPrivateCloudsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::CloudConnectionVirtualPrivateCloudsVirtualPrivateCloudsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] classic_access Value to be assigned
    def classic_access=(classic_access)
      if classic_access.nil?
        fail ArgumentError, 'classic_access cannot be nil'
      end
    ...
    def hash
      [classic_access, name, status, vpc_id].hash
    end
