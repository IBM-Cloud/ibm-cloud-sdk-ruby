...
module IbmCloudPower
  class VolumeOnboardingCreateVolumesInner < ApiModelBase
    attr_accessor :auxiliary_volumes

    # The CRN of the workspace in which the primary volume is located need to be onboarded
    attr_accessor :source_crn

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'auxiliary_volumes' => :'auxiliaryVolumes',
        :'source_crn' => :'sourceCRN'
      }
    end
    ...
    def self.openapi_types
      {
        :'auxiliary_volumes' => :'Array<AuxiliaryVolumesForOnboardingAuxiliaryVolumesInner>',
        :'source_crn' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::VolumeOnboardingCreateVolumesInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::VolumeOnboardingCreateVolumesInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'auxiliary_volumes')
        if (value = attributes[:'auxiliary_volumes']).is_a?(Array)
          self.auxiliary_volumes = value
        end
      else
        self.auxiliary_volumes = nil
      end
    ...
    def list_invalid_properties
      ...
      if @auxiliary_volumes.nil?
        invalid_properties.push('invalid value for "auxiliary_volumes", auxiliary_volumes cannot be nil.')
      end

      if @source_crn.nil?
        invalid_properties.push('invalid value for "source_crn", source_crn cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @auxiliary_volumes.nil?
      return false if @source_crn.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] auxiliary_volumes Value to be assigned
    def auxiliary_volumes=(auxiliary_volumes)
      if auxiliary_volumes.nil?
        fail ArgumentError, 'auxiliary_volumes cannot be nil'
      end

      @auxiliary_volumes = auxiliary_volumes
    end

    # Custom attribute writer method with validation
    # @param [Object] source_crn Value to be assigned
    def source_crn=(source_crn)
      if source_crn.nil?
        fail ArgumentError, 'source_crn cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          auxiliary_volumes == o.auxiliary_volumes &&
    ...
    def hash
      [auxiliary_volumes, source_crn].hash
    end
