...
module IbmCloudPower
  class NetworkSecurityGroupRuleProtocol < ApiModelBase
    ...
    def self.openapi_types
      {
        :'icmp_type' => :'String',
        :'tcp_flags' => :'Array<NetworkSecurityGroupRuleProtocolTcpFlag>',
        :'type' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::NetworkSecurityGroupRuleProtocol` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::NetworkSecurityGroupRuleProtocol`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tcp_flags')
        if (value = attributes[:'tcp_flags']).is_a?(Array)
    ...
    def valid?
      ...
      icmp_type_validator = EnumAttributeValidator.new('String', ["all", "echo", "echo-reply", "source-quench", "time-exceeded", "destination-unreach"])
      return false unless icmp_type_validator.valid?(@icmp_type)
      type_validator = EnumAttributeValidator.new('String', ["icmp", "tcp", "udp", "all"])
      return false unless type_validator.valid?(@type)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] icmp_type Object to be assigned
    def icmp_type=(icmp_type)
      validator = EnumAttributeValidator.new('String', ["all", "echo", "echo-reply", "source-quench", "time-exceeded", "destination-unreach"])
      unless validator.valid?(icmp_type)
        fail ArgumentError, "invalid value for \"icmp_type\", must be one of #{validator.allowable_values}."
      end
      @icmp_type = icmp_type
    end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["icmp", "tcp", "udp", "all"])
