...
module IbmCloudPower
  class NetworkSecurityGroup < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'default' => :'Boolean',
        :'id' => :'String',
        :'members' => :'Array<NetworkSecurityGroupMember>',
        :'name' => :'String',
        :'rules' => :'Array<NetworkSecurityGroupRule>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::NetworkSecurityGroup` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::NetworkSecurityGroup`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [crn, default, id, members, name, rules, user_tags].hash
    end
