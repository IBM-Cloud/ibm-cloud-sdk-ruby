...
module IbmCloudPower
  class RouteCreate < ApiModelBase
    # Action
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::RouteCreate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::RouteCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'destination_type')
        ...
        self.destination_type = 'ipv4-address'
      end
      ...
      if attributes.key?(:'next_hop_type')
        ...
        self.next_hop_type = 'ipv4-address'
      end
    ...
    def hash
      [action, advertise, destination, destination_type, enabled, name, next_hop, next_hop_type, user_tags].hash
    end
