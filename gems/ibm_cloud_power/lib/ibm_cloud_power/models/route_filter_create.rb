...
module IbmCloudPower
  class RouteFilterCreate < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::RouteFilterCreate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::RouteFilterCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @index > 10000000
        invalid_properties.push('invalid value for "index", must be smaller than or equal to 10000000.')
      end

      if @index < 1
        invalid_properties.push('invalid value for "index", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @index > 10000000
    ...
    def ge=(ge)
      if ge.nil?
        fail ArgumentError, 'ge cannot be nil'
      end
    ...
    def le=(le)
      if le.nil?
        fail ArgumentError, 'le cannot be nil'
      end
    ...
    def index=(index)
      ...
      if index > 10000000
        fail ArgumentError, 'invalid value for "index", must be smaller than or equal to 10000000.'
      end

      if index < 1
        fail ArgumentError, 'invalid value for "index", must be greater than or equal to 1.'
      end
    ...
    def hash
      [ge, le, action, direction, index, prefix].hash
    end
