...
module IbmCloudPower
  class WorkspacePowerEdgeRouterDetails < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::WorkspacePowerEdgeRouterDetails` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::WorkspacePowerEdgeRouterDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      migration_status_validator = EnumAttributeValidator.new('String', ["intializing", "migrating", "deleted", "completed"])
      return false unless migration_status_validator.valid?(@migration_status)
      return false if @state.nil?
      state_validator = EnumAttributeValidator.new('String', ["active", "error", "warning", "configuring", "removing", "inactive", "user-validation", "inaccessible"])
      return false unless state_validator.valid?(@state)
      return false if @type.nil?
      type_validator = EnumAttributeValidator.new('String', ["automated", "manual"])
      return false unless type_validator.valid?(@type)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] migration_status Object to be assigned
    def migration_status=(migration_status)
      validator = EnumAttributeValidator.new('String', ["intializing", "migrating", "deleted", "completed"])
      unless validator.valid?(migration_status)
        fail ArgumentError, "invalid value for \"migration_status\", must be one of #{validator.allowable_values}."
      end
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["active", "error", "warning", "configuring", "removing", "inactive", "user-validation", "inaccessible"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["automated", "manual"])
    ...
    def hash
      [migration_status, state, type].hash
    end
