...
module IbmCloudPower
  class NetworkInterface < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::NetworkInterface` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::NetworkInterface`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'network_security_group_id')
        self.network_security_group_id = attributes[:'network_security_group_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          name == o.name &&
          network_security_group_id == o.network_security_group_id &&
    ...
    def hash
      [crn, external_ip, id, instance, ip_address, mac_address, name, network_security_group_id, network_security_group_ids, status, user_tags].hash
    end
