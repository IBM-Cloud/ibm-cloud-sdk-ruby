...
module IbmCloudPower
  class ImageSpecifications < ApiModelBase
    # Architecture
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::ImageSpecifications` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::ImageSpecifications`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'container_format')
        self.container_format = attributes[:'container_format']
      end

      if attributes.key?(:'disk_format')
        self.disk_format = attributes[:'disk_format']
      end

      if attributes.key?(:'endianness')
        self.endianness = attributes[:'endianness']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          container_format == o.container_format &&
          disk_format == o.disk_format &&
    ...
    def hash
      [architecture, container_format, disk_format, endianness, hypervisor_type, image_type, operating_system].hash
    end
