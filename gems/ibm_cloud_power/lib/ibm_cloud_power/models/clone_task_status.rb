...
module IbmCloudPower
  class CloneTaskStatus < ApiModelBase
    ...
    def self.openapi_types
      {
        :'cloned_volumes' => :'Array<ClonedVolume>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::CloneTaskStatus` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::CloneTaskStatus`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["running", "completed", "failed", "unknown"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["running", "completed", "failed", "unknown"])
    ...
    def hash
      [cloned_volumes, failed_reason, percent_complete, status].hash
    end
