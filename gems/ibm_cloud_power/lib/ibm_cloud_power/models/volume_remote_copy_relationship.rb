...
module IbmCloudPower
  class VolumeRemoteCopyRelationship < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::VolumeRemoteCopyRelationship`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [aux_changed_volume_name, aux_volume_name, consistency_group_name, copy_type, cycling_mode, freeze_time, master_changed_volume_name, master_volume_name, name, primary_role, progress, remote_copy_id, state, sync, cycle_period_seconds, id].hash
    end
