...
module IbmCloudPower
  class PvmInstanceDeployment < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::PvmInstanceDeployment` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::PvmInstanceDeployment`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'processor_mode')
        self.processor_mode = attributes[:'processor_mode']
      else
        self.processor_mode = 'dedicated'
      end
    ...
    def list_invalid_properties
      ...
      if @processor_mode.nil?
        invalid_properties.push('invalid value for "processor_mode", processor_mode cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @processor_mode.nil?
      processor_mode_validator = EnumAttributeValidator.new('String', ["dedicated", "shared", "capped"])
      return false unless processor_mode_validator.valid?(@processor_mode)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] processor_mode Object to be assigned
    def processor_mode=(processor_mode)
      validator = EnumAttributeValidator.new('String', ["dedicated", "shared", "capped"])
      unless validator.valid?(processor_mode)
        fail ArgumentError, "invalid value for \"processor_mode\", must be one of #{validator.allowable_values}."
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          processor_mode == o.processor_mode &&
          type == o.type
    end
    ...
    def hash
      [machine_type, pin_policy, processor_mode, type].hash
    end
