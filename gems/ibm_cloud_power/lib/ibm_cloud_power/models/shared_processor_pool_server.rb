...
module IbmCloudPower
  class SharedProcessorPoolServer < ApiModelBase
    # The amount of cpus for the server
    attr_accessor :cpus

    # Identifies if uncapped or not
    attr_accessor :uncapped
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::SharedProcessorPoolServer` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::SharedProcessorPoolServer`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'uncapped')
        self.uncapped = attributes[:'uncapped']
      end
    ...
    def hash
      [cpus, uncapped, availability_zone, id, memory, name, status, vcpus].hash
    end
