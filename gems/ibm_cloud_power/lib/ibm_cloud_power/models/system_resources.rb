...
module IbmCloudPower
  # Information about the resources that a single system contains
  class SystemResources < ApiModelBase
    # The host available Processor units
    attr_accessor :cores

    # The host MTMS name
    attr_accessor :host_mtms_name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::SystemResources` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::SystemResources`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'host_mtms_name')
        self.host_mtms_name = attributes[:'host_mtms_name']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          cores == o.cores &&
          host_mtms_name == o.host_mtms_name &&
          id == o.id &&
          memory == o.memory &&
    ...
    def hash
      [cores, host_mtms_name, id, memory, total_cores, total_memory, total_phys_cores, total_phys_memory].hash
    end
