  ...
  class Route < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::Route` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::Route`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @advertise.nil?
        invalid_properties.push('invalid value for "advertise", advertise cannot be nil.')
      end
      ...
      if @next_hop_type.nil?
        invalid_properties.push('invalid value for "next_hop_type", next_hop_type cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @name !~ Regexp.new(/^[\s]*[A-Za-z0-9:_.\-][A-Za-z0-9\s:_.\-]*$/)
      return false if @next_hop.nil?
      return false if @next_hop_type.nil?
      ...
      state_validator = EnumAttributeValidator.new('String', ["defined", "deployed", "disabled", "error", "deploying", "undeploying", "deleting"])
    ...
    def action=(action)
      validator = EnumAttributeValidator.new('String', ["deliver"])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] advertise Object to be assigned
    def advertise=(advertise)
      validator = EnumAttributeValidator.new('String', ["enable", "disable"])
      unless validator.valid?(advertise)
        fail ArgumentError, "invalid value for \"advertise\", must be one of #{validator.allowable_values}."
      end
      @advertise = advertise
    end
    ...
    def destination_type=(destination_type)
      validator = EnumAttributeValidator.new('String', ["ipv4-address"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] next_hop Value to be assigned
    def next_hop=(next_hop)
      if next_hop.nil?
        fail ArgumentError, 'next_hop cannot be nil'
      end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] next_hop_type Object to be assigned
    def next_hop_type=(next_hop_type)
      validator = EnumAttributeValidator.new('String', ["ipv4-address"])
      unless validator.valid?(next_hop_type)
        fail ArgumentError, "invalid value for \"next_hop_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["defined", "deployed", "disabled", "error", "deploying", "undeploying", "deleting"])
    ...
    def hash
      [action, advertise, crn, destination, destination_type, enabled, id, name, next_hop, next_hop_type, state, user_tags].hash
    end
