...
module IbmCloudPower
  class AuxiliaryVolumesForOnboardingAuxiliaryVolumesInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::AuxiliaryVolumesForOnboardingAuxiliaryVolumesInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::AuxiliaryVolumesForOnboardingAuxiliaryVolumesInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @aux_volume_name.nil?
        invalid_properties.push('invalid value for "aux_volume_name", aux_volume_name cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @aux_volume_name.nil?
    ...
    # @param [Object] aux_volume_name Value to be assigned
    def aux_volume_name=(aux_volume_name)
      if aux_volume_name.nil?
        fail ArgumentError, 'aux_volume_name cannot be nil'
      end

      @aux_volume_name = aux_volume_name
    end
