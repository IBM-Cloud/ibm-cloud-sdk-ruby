...
module IbmCloudPower
  class ImageImportDetails < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::ImageImportDetails` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::ImageImportDetails`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      license_type_validator = EnumAttributeValidator.new('String', ["byol"])
      return false unless license_type_validator.valid?(@license_type)
      return false if @product.nil?
      product_validator = EnumAttributeValidator.new('String', ["Hana", "Netweaver"])
      return false unless product_validator.valid?(@product)
      return false if @vendor.nil?
      vendor_validator = EnumAttributeValidator.new('String', ["SAP"])
    ...
    def license_type=(license_type)
      validator = EnumAttributeValidator.new('String', ["byol"])
    ...
    def product=(product)
      validator = EnumAttributeValidator.new('String', ["Hana", "Netweaver"])
    ...
    def vendor=(vendor)
      validator = EnumAttributeValidator.new('String', ["SAP"])
    ...
    def hash
      [license_type, product, vendor].hash
    end
