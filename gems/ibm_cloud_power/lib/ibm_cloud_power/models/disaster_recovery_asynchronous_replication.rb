...
module IbmCloudPower
  class DisasterRecoveryAsynchronousReplication < ApiModelBase
    # Service Enabled
    ...
    def self.openapi_types
      {
        :'enabled' => :'Boolean',
        :'target_locations' => :'Array<ReplicationTargetLocation>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::DisasterRecoveryAsynchronousReplication` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::DisasterRecoveryAsynchronousReplication`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'target_locations')
        if (value = attributes[:'target_locations']).is_a?(Array)
          self.target_locations = value
        end
      else
        self.target_locations = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @target_locations.nil?
        invalid_properties.push('invalid value for "target_locations", target_locations cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @target_locations.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] target_locations Value to be assigned
    def target_locations=(target_locations)
      if target_locations.nil?
        fail ArgumentError, 'target_locations cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          enabled == o.enabled &&
          target_locations == o.target_locations
    end
