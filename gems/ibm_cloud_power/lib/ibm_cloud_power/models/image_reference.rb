...
module IbmCloudPower
  class ImageReference < ApiModelBase
    # Creation Date
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::ImageReference`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] specifications Value to be assigned
    def specifications=(specifications)
      if specifications.nil?
        fail ArgumentError, 'specifications cannot be nil'
      end
    ...
    def hash
      [creation_date, crn, description, href, image_id, last_update_date, name, specifications, state, storage_pool, storage_type].hash
    end
