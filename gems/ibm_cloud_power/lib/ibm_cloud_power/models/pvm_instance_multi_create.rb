...
module IbmCloudPower
  class PVMInstanceMultiCreate < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::PVMInstanceMultiCreate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::PVMInstanceMultiCreate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'numerical')
        self.numerical = attributes[:'numerical']
      else
        self.numerical = 'suffix'
      end
    end
    ...
    def valid?
      ...
      affinity_policy_validator = EnumAttributeValidator.new('String', ["affinity", "anti-affinity", "none"])
      return false unless affinity_policy_validator.valid?(@affinity_policy)
      numerical_validator = EnumAttributeValidator.new('String', ["prefix", "suffix"])
      return false unless numerical_validator.valid?(@numerical)
      true
    end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] numerical Object to be assigned
    def numerical=(numerical)
      validator = EnumAttributeValidator.new('String', ["prefix", "suffix"])
      unless validator.valid?(numerical)
        fail ArgumentError, "invalid value for \"numerical\", must be one of #{validator.allowable_values}."
      end
      @numerical = numerical
    end
    ...
    def hash
      [affinity_policy, count, numerical].hash
    end
