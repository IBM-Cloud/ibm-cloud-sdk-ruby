...
module IbmCloudPower
  class PeerInterface < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::PeerInterface` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::PeerInterface`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'peer_type')
        self.peer_type = attributes[:'peer_type']
      else
        self.peer_type = nil
      end
    ...
    def list_invalid_properties
      ...
      if @peer_type.nil?
        invalid_properties.push('invalid value for "peer_type", peer_type cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @peer_type.nil?
      peer_type_validator = EnumAttributeValidator.new('String', ["interface", "lag"])
      return false unless peer_type_validator.valid?(@peer_type)
    ...
    def peer_type=(peer_type)
      validator = EnumAttributeValidator.new('String', ["interface", "lag"])
      unless validator.valid?(peer_type)
        fail ArgumentError, "invalid value for \"peer_type\", must be one of #{validator.allowable_values}."
      end
      @peer_type = peer_type
    end
    ...
    def hash
      [device_id, name, peer_interface_id, peer_type, port_id].hash
    end
