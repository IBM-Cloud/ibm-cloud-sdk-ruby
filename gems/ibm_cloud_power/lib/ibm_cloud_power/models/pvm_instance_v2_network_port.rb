...
module IbmCloudPower
  # PVM's Port information
  class PVMInstanceV2NetworkPort < ApiModelBase
    # Unique Port ID
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudPower::PVMInstanceV2NetworkPort` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudPower::PVMInstanceV2NetworkPort`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      ip_protocol_validator = EnumAttributeValidator.new('String', ["IPv4", "IPv6"])
      return false unless ip_protocol_validator.valid?(@ip_protocol)
      type_validator = EnumAttributeValidator.new('String', ["dhcp", "static"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["dhcp", "static"])
    ...
    def hash
      [id, ip_protocol, mac_address, private_ip, type].hash
    end
