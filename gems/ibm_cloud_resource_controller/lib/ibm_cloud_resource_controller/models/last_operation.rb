...
module IbmCloudResourceController
  # An element of the last operation of the instance.
  class LastOperation < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudResourceController::LastOperation` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudResourceController::LastOperation`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @poll.nil?
        invalid_properties.push('invalid value for "poll", poll cannot be nil.')
      end
    ...
    def valid?
      ...
      state_validator = EnumAttributeValidator.new('String', ["in progress", "succeeded", "failed"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["in progress", "succeeded", "failed"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] cancelable Value to be assigned
    def cancelable=(cancelable)
      if cancelable.nil?
        fail ArgumentError, 'cancelable cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] poll Value to be assigned
    def poll=(poll)
      if poll.nil?
        fail ArgumentError, 'poll cannot be nil'
      end
    ...
    def hash
      [type, state, sub_type, async, description, reason_code, poll_after, cancelable, poll].hash
    end
