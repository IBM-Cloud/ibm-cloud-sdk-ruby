...
module IbmCloudResourceController
  ...
  class ResourceKeyPostParameters < ApiModelBase
    ...
    # An optional Boolean that determines whether the created resource key credentials can be retrieved by using get resource key or get a list of all the resource keys requests. The default value is 'onetime_credentials' from the instance.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudResourceController::ResourceKeyPostParameters` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudResourceController::ResourceKeyPostParameters`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [serviceid_crn, onetime_credentials].hash
    end
