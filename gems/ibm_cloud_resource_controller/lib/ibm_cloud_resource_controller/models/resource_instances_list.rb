...
module IbmCloudResourceController
  ...
  class ResourceInstancesList < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudResourceController::ResourceInstancesList`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] rows_count Value to be assigned
    def rows_count=(rows_count)
      if rows_count.nil?
        fail ArgumentError, 'rows_count cannot be nil'
      end
    ...
    def hash
      [rows_count, next_url, resources].hash
    end
