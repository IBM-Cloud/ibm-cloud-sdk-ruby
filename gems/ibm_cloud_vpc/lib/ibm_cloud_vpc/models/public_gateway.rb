  ...
  class PublicGateway
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudVpc::PublicGateway` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudVpc::PublicGateway`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @floating_ip.nil?
        invalid_properties.push('invalid value for "floating_ip", floating_ip cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @floating_ip.nil?
      ...
      resource_type_validator = EnumAttributeValidator.new('String', ["public_gateway"])
    ...
    def resource_type=(resource_type)
      validator = EnumAttributeValidator.new('String', ["public_gateway"])
    ...
    def hash
      [created_at, crn, floating_ip, href, id, name, resource_group, resource_type, status, vpc, zone].hash
    end
