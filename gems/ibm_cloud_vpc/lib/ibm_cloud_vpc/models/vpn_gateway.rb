  ...
  class VPNGateway
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudVpc::VPNGateway` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudVpc::VPNGateway`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      resource_type_validator = EnumAttributeValidator.new('String', ["vpn_gateway"])
    ...
    def resource_type=(resource_type)
      validator = EnumAttributeValidator.new('String', ["vpn_gateway"])
    ...
    def hash
      [connections, created_at, crn, href, id, name, public_ip, resource_group, resource_type, status, subnet].hash
    end
