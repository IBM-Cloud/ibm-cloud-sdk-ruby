  ...
  class InstanceAction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudVpc::InstanceAction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudVpc::InstanceAction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["completed", "failed", "pending", "running"])
      return false unless status_validator.valid?(@status)
      return false if @type.nil?
      type_validator = EnumAttributeValidator.new('String', ["reboot", "start", "stop"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["completed", "failed", "pending", "running"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["reboot", "start", "stop"])
    ...
    def hash
      [completed_at, created_at, force, href, id, started_at, status, type].hash
    end
