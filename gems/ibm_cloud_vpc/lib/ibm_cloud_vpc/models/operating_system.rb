  ...
  class OperatingSystem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudVpc::OperatingSystem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudVpc::OperatingSystem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @display_name.to_s.length > 120
      return false if @display_name.to_s.length < 1
      return false if @display_name !~ Regexp.new(/^[-A-Za-z0-9 !$@#%&*'=_+:;,?\.\/\(\)\[\]]+$/)
    ...
    def hash
      [architecture, display_name, family, href, name, vendor, version].hash
    end
