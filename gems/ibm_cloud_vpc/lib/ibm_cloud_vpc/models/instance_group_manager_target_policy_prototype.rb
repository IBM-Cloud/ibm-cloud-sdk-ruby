...
module IbmCloudVpc
  class InstanceGroupManagerTargetPolicyPrototype
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudVpc::InstanceGroupManagerTargetPolicyPrototype` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudVpc::InstanceGroupManagerTargetPolicyPrototype`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      metric_type_validator = EnumAttributeValidator.new('String', ["cpu", "memory", "network_in", "network_out"])
      ...
      policy_type_validator = EnumAttributeValidator.new('String', ["target"])
    ...
    def metric_type=(metric_type)
      validator = EnumAttributeValidator.new('String', ["cpu", "memory", "network_in", "network_out"])
    ...
    def policy_type=(policy_type)
      validator = EnumAttributeValidator.new('String', ["target"])
    ...
    def hash
      [metric_type, metric_value, policy_type].hash
    end
