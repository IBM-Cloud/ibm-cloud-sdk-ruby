...
module IbmCloudVpc
  class LoadBalancerListenerPolicyRedirectURLPrototype
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudVpc::LoadBalancerListenerPolicyRedirectURLPrototype` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudVpc::LoadBalancerListenerPolicyRedirectURLPrototype`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] http_status_code Object to be assigned
    def http_status_code=(http_status_code)
      validator = EnumAttributeValidator.new('Integer', [301, 302, 303, 307, 308])
      unless validator.valid?(http_status_code)
        fail ArgumentError, "invalid value for \"http_status_code\", must be one of #{validator.allowable_values}."
      end
      @http_status_code = http_status_code
    end
