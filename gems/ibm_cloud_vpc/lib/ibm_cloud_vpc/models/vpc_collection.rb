  ...
  class VPCCollection
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudVpc::VPCCollection` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudVpc::VPCCollection`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'vpcs')
        if (value = attributes[:'vpcs']).is_a?(Array)
          self.vpcs = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @vpcs.nil?
        invalid_properties.push('invalid value for "vpcs", vpcs cannot be nil.')
      end
    ...
    def hash
      [first, limit, _next, total_count, vpcs].hash
    end
