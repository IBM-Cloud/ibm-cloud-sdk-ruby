...
module IbmCloudIam
  class TemplateAssignmentResponseResourceDetail < ApiModelBase
    ...
    attr_accessor :resource_created
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::TemplateAssignmentResponseResourceDetail` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::TemplateAssignmentResponseResourceDetail`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'resource_created')
        self.resource_created = attributes[:'resource_created']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          id == o.id &&
          version == o.version &&
          resource_created == o.resource_created &&
    ...
    def hash
      [id, version, resource_created, error_message, status].hash
    end
