...
module IbmCloudIam
  ...
  class CreateTemplateAssignmentRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::CreateTemplateAssignmentRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::CreateTemplateAssignmentRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @template_version < 1
      return false if @target_type.nil?
      target_type_validator = EnumAttributeValidator.new('String', ["Account", "AccountGroup"])
    ...
    def target_type=(target_type)
      validator = EnumAttributeValidator.new('String', ["Account", "AccountGroup"])
    ...
    def hash
      [template_id, template_version, target_type, target].hash
    end
