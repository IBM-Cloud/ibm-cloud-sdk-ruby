...
module IbmCloudIam
  ...
  class CreateIdpRequestSecretsIdp < ApiModelBase
    ...
    def self.openapi_types
      {
        :'xml_import' => :'Boolean',
        :'signing' => :'Array<CreateIdpRequestSecretsIdpSigningInner>',
        :'encrypting' => :'Array<CreateIdpRequestSecretsIdpSigningInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::CreateIdpRequestSecretsIdp` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::CreateIdpRequestSecretsIdp`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'encrypting')
        if (value = attributes[:'encrypting']).is_a?(Array)
          self.encrypting = value
        end
      end
    end
    ...
    def hash
      [xml_import, signing, encrypting].hash
    end
