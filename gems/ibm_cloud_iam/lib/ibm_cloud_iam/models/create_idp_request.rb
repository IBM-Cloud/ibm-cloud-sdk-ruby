...
module IbmCloudIam
  class CreateIdpRequest < ApiModelBase
    ...
    # Defines if the IDP is active (enabled) for all accounts (including those who consumed the IdP). Default during creation is true
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::CreateIdpRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::CreateIdpRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["ldap", "appid", "saml"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["ldap", "appid", "saml"])
    ...
    def hash
      [account_id, name, type, active, properties, secrets, share_scope].hash
    end
