...
module IbmCloudIam
  class Activity < ApiModelBase
    ...
    attr_accessor :last_authn

    # Authentication count, number of times the entity was authenticated.
    attr_accessor :authn_count

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'last_authn' => :'last_authn',
        :'authn_count' => :'authn_count'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::Activity` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::Activity`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'last_authn')
        self.last_authn = attributes[:'last_authn']
      end

      if attributes.key?(:'authn_count')
        self.authn_count = attributes[:'authn_count']
      else
        self.authn_count = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @authn_count.nil?
        invalid_properties.push('invalid value for "authn_count", authn_count cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @authn_count.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] authn_count Value to be assigned
    def authn_count=(authn_count)
      if authn_count.nil?
        fail ArgumentError, 'authn_count cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          last_authn == o.last_authn &&
          authn_count == o.authn_count
    end
