  ...
  class TestResult < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'modified_at' => :'String',
        :'idp_version' => :'String',
        :'steps' => :'Array<TestResultStepsInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::TestResult` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::TestResult`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'idp_version')
        self.idp_version = attributes[:'idp_version']
      end
    ...
    def hash
      [idp_id, entity_tag, started_at, modified_at, idp_version, steps].hash
    end
