...
module IbmCloudIam
  ...
  class TrustedProfileTemplateResponse < ApiModelBase
    ...
    attr_accessor :policy_template_references

    attr_accessor :action_controls
    ...
    def self.openapi_types
      {
        ...
        :'policy_template_references' => :'Array<PolicyTemplateReference>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::TrustedProfileTemplateResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::TrustedProfileTemplateResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'policy_template_references')
        if (value = attributes[:'policy_template_references']).is_a?(Array)
          self.policy_template_references = value
        end
      end

      if attributes.key?(:'action_controls')
        self.action_controls = attributes[:'action_controls']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          profile == o.profile &&
          policy_template_references == o.policy_template_references &&
          action_controls == o.action_controls &&
          history == o.history &&
    ...
    def hash
      [id, version, account_id, name, description, committed, profile, policy_template_references, action_controls, history, entity_tag, crn, created_at, created_by_id, last_modified_at, last_modified_by_id].hash
    end
