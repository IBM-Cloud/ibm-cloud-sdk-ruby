...
module IbmCloudIam
  ...
  class ServiceIdList < ApiModelBase
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::ServiceIdList`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] serviceids Value to be assigned
    def serviceids=(serviceids)
      if serviceids.nil?
        fail ArgumentError, 'serviceids cannot be nil'
      end
