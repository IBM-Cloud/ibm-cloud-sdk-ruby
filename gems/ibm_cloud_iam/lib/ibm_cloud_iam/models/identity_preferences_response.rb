...
module IbmCloudIam
  class IdentityPreferencesResponse < ApiModelBase
    ...
    def self.openapi_types
      {
        :'preferences' => :'Array<IdentityPreferenceResponse>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::IdentityPreferencesResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::IdentityPreferencesResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] preferences Value to be assigned
    def preferences=(preferences)
      if preferences.nil?
        fail ArgumentError, 'preferences cannot be nil'
      end
