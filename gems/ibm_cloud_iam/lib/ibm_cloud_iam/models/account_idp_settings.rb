...
module IbmCloudIam
  class AccountIdpSettings < ApiModelBase
    attr_accessor :idp_id

    attr_accessor :owner_account

    attr_accessor :owner_account_name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::AccountIdpSettings` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::AccountIdpSettings`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'owner_account_name')
        self.owner_account_name = attributes[:'owner_account_name']
      end
      ...
      if attributes.key?(:'idp_type')
        self.idp_type = attributes[:'idp_type']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          owner_account_name == o.owner_account_name &&
          idp_name == o.idp_name &&
          idp_type == o.idp_type &&
    ...
    def hash
      [idp_id, owner_account, owner_account_name, idp_name, idp_type, cloud_user_strategy, active, ui_default].hash
    end
