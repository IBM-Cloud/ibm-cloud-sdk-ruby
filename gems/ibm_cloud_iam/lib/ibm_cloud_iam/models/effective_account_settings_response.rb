...
module IbmCloudIam
  ...
  class EffectiveAccountSettingsResponse < ApiModelBase
    ...
    attr_accessor :assigned_templates
    ...
    def self.openapi_types
      {
        ...
        :'assigned_templates' => :'Array<AccountSettingsAssignedTemplatesSection>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::EffectiveAccountSettingsResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::EffectiveAccountSettingsResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'assigned_templates')
        if (value = attributes[:'assigned_templates']).is_a?(Array)
          self.assigned_templates = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @effective.nil?
        invalid_properties.push('invalid value for "effective", effective cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] effective Value to be assigned
    def effective=(effective)
      if effective.nil?
        fail ArgumentError, 'effective cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          account == o.account &&
          assigned_templates == o.assigned_templates
    end
    ...
    def hash
      [context, account_id, effective, account, assigned_templates].hash
    end
