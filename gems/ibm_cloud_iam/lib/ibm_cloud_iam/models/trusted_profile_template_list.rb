...
module IbmCloudIam
  class TrustedProfileTemplateList < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'previous' => :'String',
        :'_next' => :'String',
        :'profile_templates' => :'Array<TrustedProfileTemplateResponse>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::TrustedProfileTemplateList` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::TrustedProfileTemplateList`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'profile_templates')
        if (value = attributes[:'profile_templates']).is_a?(Array)
          self.profile_templates = value
        end
      else
        self.profile_templates = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @profile_templates.nil?
        invalid_properties.push('invalid value for "profile_templates", profile_templates cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @profile_templates.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] profile_templates Value to be assigned
    def profile_templates=(profile_templates)
      if profile_templates.nil?
        fail ArgumentError, 'profile_templates cannot be nil'
      end
    ...
    def hash
      [context, offset, limit, first, previous, _next, profile_templates].hash
    end
