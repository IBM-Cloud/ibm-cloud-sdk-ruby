...
module IbmCloudIam
  class CreateProfileLinkRequest < ApiModelBase
    ...
    attr_accessor :is_cross_account
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::CreateProfileLinkRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::CreateProfileLinkRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_cross_account')
        self.is_cross_account = attributes[:'is_cross_account']
      end
    ...
    def list_invalid_properties
      ...
      if @cr_type.nil?
        invalid_properties.push('invalid value for "cr_type", cr_type cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @cr_type.nil?
      return false if @link.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] cr_type Value to be assigned
    def cr_type=(cr_type)
      if cr_type.nil?
        fail ArgumentError, 'cr_type cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          name == o.name &&
          cr_type == o.cr_type &&
          is_cross_account == o.is_cross_account &&
          link == o.link
    end
    ...
    def hash
      [name, cr_type, is_cross_account, link].hash
    end
