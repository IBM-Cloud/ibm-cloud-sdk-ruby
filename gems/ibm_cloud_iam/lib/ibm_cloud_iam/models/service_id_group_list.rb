...
module IbmCloudIam
  class ServiceIdGroupList < ApiModelBase
    ...
    def self.openapi_types
      {
        :'serviceid_groups' => :'Array<ServiceIdGroup>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::ServiceIdGroupList` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::ServiceIdGroupList`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'serviceid_groups')
        if (value = attributes[:'serviceid_groups']).is_a?(Array)
          self.serviceid_groups = value
        end
      else
        self.serviceid_groups = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @serviceid_groups.nil?
        invalid_properties.push('invalid value for "serviceid_groups", serviceid_groups cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @serviceid_groups.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] serviceid_groups Value to be assigned
    def serviceid_groups=(serviceid_groups)
      if serviceid_groups.nil?
        fail ArgumentError, 'serviceid_groups cannot be nil'
      end
