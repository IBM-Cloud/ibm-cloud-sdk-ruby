...
module IbmCloudIam
  class AccountSettingsTemplateList < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'previous' => :'String',
        :'_next' => :'String',
        :'account_settings_templates' => :'Array<AccountSettingsTemplateResponse>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::AccountSettingsTemplateList` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::AccountSettingsTemplateList`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'account_settings_templates')
        if (value = attributes[:'account_settings_templates']).is_a?(Array)
          self.account_settings_templates = value
        end
      else
        self.account_settings_templates = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @account_settings_templates.nil?
        invalid_properties.push('invalid value for "account_settings_templates", account_settings_templates cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @account_settings_templates.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] account_settings_templates Value to be assigned
    def account_settings_templates=(account_settings_templates)
      if account_settings_templates.nil?
        fail ArgumentError, 'account_settings_templates cannot be nil'
      end
    ...
    def hash
      [context, offset, limit, first, previous, _next, account_settings_templates].hash
    end
