...
module IbmCloudIam
  class SamlMetadataImportResponse < ApiModelBase
    ...
    attr_accessor :ui_setup_completed
    ...
    def idp_id=(idp_id)
      ...
      if idp_id.to_s.length > 10
        fail ArgumentError, 'invalid value for "idp_id", the character length must be smaller than or equal to 10.'
      end

      if idp_id.to_s.length < 3
        fail ArgumentError, 'invalid value for "idp_id", the character length must be greater than or equal to 3.'
      end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["saml"])
    ...
    def hash
      [idp_id, entity_tag, created_at, modified_at, account_id, name, type, properties, secrets, history, share_scope, active, ui_setup_completed].hash
    end
