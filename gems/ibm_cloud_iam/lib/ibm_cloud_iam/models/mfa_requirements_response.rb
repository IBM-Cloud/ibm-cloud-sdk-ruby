  ...
  class MFARequirementsResponse < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::MFARequirementsResponse` initialize method"
      end
    ...
    def hash
      [error, code, authorization_token].hash
    end
