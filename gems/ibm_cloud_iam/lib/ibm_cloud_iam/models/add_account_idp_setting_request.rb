...
module IbmCloudIam
  class AddAccountIdpSettingRequest < ApiModelBase
    # Strategy how Cloud User representives for the IdP users are handled
    attr_accessor :cloud_user_strategy
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::AddAccountIdpSettingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::AddAccountIdpSettingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'cloud_user_strategy')
        self.cloud_user_strategy = attributes[:'cloud_user_strategy']
      else
        self.cloud_user_strategy = nil
      end
      ...
      if attributes.key?(:'ui_default')
        self.ui_default = attributes[:'ui_default']
      else
        self.ui_default = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @cloud_user_strategy.nil?
        invalid_properties.push('invalid value for "cloud_user_strategy", cloud_user_strategy cannot be nil.')
      end
      ...
      if @ui_default.nil?
        invalid_properties.push('invalid value for "ui_default", ui_default cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @cloud_user_strategy.nil?
      cloud_user_strategy_validator = EnumAttributeValidator.new('String', ["STATIC", "DYNAMIC", "NEVER"])
      return false unless cloud_user_strategy_validator.valid?(@cloud_user_strategy)
      return false if @active.nil?
      return false if @ui_default.nil?
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] cloud_user_strategy Object to be assigned
    def cloud_user_strategy=(cloud_user_strategy)
      validator = EnumAttributeValidator.new('String', ["STATIC", "DYNAMIC", "NEVER"])
      unless validator.valid?(cloud_user_strategy)
        fail ArgumentError, "invalid value for \"cloud_user_strategy\", must be one of #{validator.allowable_values}."
      end
      @cloud_user_strategy = cloud_user_strategy
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] ui_default Value to be assigned
    def ui_default=(ui_default)
      if ui_default.nil?
        fail ArgumentError, 'ui_default cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          cloud_user_strategy == o.cloud_user_strategy &&
          active == o.active &&
          ui_default == o.ui_default
    end
    ...
    def hash
      [cloud_user_strategy, active, ui_default].hash
    end
