...
module IbmCloudIam
  ...
  class TrustedProfile < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::TrustedProfile` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::TrustedProfile`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ims_user_id')
        self.ims_user_id = attributes[:'ims_user_id']
      end
    ...
    def hash
      [context, id, entity_tag, crn, name, description, email, created_at, modified_at, iam_id, account_id, template_id, assignment_id, ims_account_id, ims_user_id, history, activity].hash
    end
