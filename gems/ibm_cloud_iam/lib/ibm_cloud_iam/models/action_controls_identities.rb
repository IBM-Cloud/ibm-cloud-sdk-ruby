...
module IbmCloudIam
  class ActionControlsIdentities < ApiModelBase
    attr_accessor :add
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudIam::ActionControlsIdentities` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudIam::ActionControlsIdentities`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @add.nil?
        invalid_properties.push('invalid value for "add", add cannot be nil.')
      end

      if @remove.nil?
        invalid_properties.push('invalid value for "remove", remove cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @add.nil?
      return false if @remove.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] add Value to be assigned
    def add=(add)
      if add.nil?
        fail ArgumentError, 'add cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] remove Value to be assigned
    def remove=(remove)
      if remove.nil?
        fail ArgumentError, 'remove cannot be nil'
      end
