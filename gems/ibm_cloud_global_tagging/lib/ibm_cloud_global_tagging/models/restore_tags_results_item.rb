...
module IbmCloudGlobalTagging
  # Item of the restore_tags results.
  class RestoreTagsResultsItem < ApiModelBase
    # resource_id.
    attr_accessor :resource_id

    # list of user tags restored.
    attr_accessor :tags

    # list of service tags restored.
    attr_accessor :service_tags

    # list of access tags restored.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudGlobalTagging::RestoreTagsResultsItem` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudGlobalTagging::RestoreTagsResultsItem`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [resource_id, tags, service_tags, access_tags].hash
    end
