  ...
  class Resource < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudGlobalTagging::Resource` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudGlobalTagging::Resource`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @resource_id.to_s.length > 1024
        invalid_properties.push('invalid value for "resource_id", the character length must be smaller than or equal to 1024.')
      end
      ...
      if !@resource_type.nil? && @resource_type !~ pattern
        invalid_properties.push("invalid value for \"resource_type\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if @resource_id.to_s.length > 1024
      return false if @resource_id !~ Regexp.new(/^(crn:v1(:[^:]*){7}:.*|[a-zA-Z0-9_\-\.]+)$/)
      return false if !@resource_type.nil? && @resource_type !~ Regexp.new(/^[A-Za-z0-9_ :.-]*$/)
      true
    end
    ...
    def resource_id=(resource_id)
      ...
      if resource_id.to_s.length > 1024
        fail ArgumentError, 'invalid value for "resource_id", the character length must be smaller than or equal to 1024.'
      end
    ...
    def resource_type=(resource_type)
      ...
      if resource_type !~ pattern
        fail ArgumentError, "invalid value for \"resource_type\", must conform to the pattern #{pattern}."
      end
