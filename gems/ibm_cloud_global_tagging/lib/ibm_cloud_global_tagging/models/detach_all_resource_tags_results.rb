...
module IbmCloudGlobalTagging
  ...
  class DetachAllResourceTagsResults < ApiModelBase
    # The total number of detached tags
    attr_accessor :tot_detached_tags

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'tot_detached_tags' => :'tot_detached_tags'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudGlobalTagging::DetachAllResourceTagsResults` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudGlobalTagging::DetachAllResourceTagsResults`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'tot_detached_tags')
        self.tot_detached_tags = attributes[:'tot_detached_tags']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          tot_detached_tags == o.tot_detached_tags
    end
