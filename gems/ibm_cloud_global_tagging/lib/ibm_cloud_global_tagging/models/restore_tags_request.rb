...
module IbmCloudGlobalTagging
  # Array of crns.
  class RestoreTagsRequest < ApiModelBase
    # An array of resources to restore tags to
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudGlobalTagging::RestoreTagsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudGlobalTagging::RestoreTagsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @resources.length < 0
        invalid_properties.push('invalid value for "resources", number of items must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @resources.length > 100
      return false if @resources.length < 0
      true
    end
    ...
    def resources=(resources)
      ...
      if resources.length < 0
        fail ArgumentError, 'invalid value for "resources", number of items must be greater than or equal to 0.'
      end
