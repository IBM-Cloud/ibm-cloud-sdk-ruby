...
module IbmCloudGlobalTagging
  # Array of tag names and resource ID on which the tags are set.
  class SetTagsRequest < ApiModelBase
    # An array of tag names to set
    attr_accessor :tag_names

    # List of resources on which the tags are set. Current implementation support only one resource whose type is included in a specific subset of IMS resources.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IbmCloudGlobalTagging::SetTagsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IbmCloudGlobalTagging::SetTagsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @tag_names.nil?
        invalid_properties.push('invalid value for "tag_names", tag_names cannot be nil.')
      end

      if @tag_names.length < 0
        invalid_properties.push('invalid value for "tag_names", number of items must be greater than or equal to 0.')
      end
      ...
      if @resources.length > 1
        invalid_properties.push('invalid value for "resources", number of items must be less than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @tag_names.nil?
      return false if @tag_names.length < 0
      return false if @resources.nil?
      return false if @resources.length > 1
      return false if @resources.length < 1
      true
    end
    ...
    def tag_names=(tag_names)
      ...
      if tag_names.length < 0
        fail ArgumentError, 'invalid value for "tag_names", number of items must be greater than or equal to 0.'
      end
    ...
    def resources=(resources)
      ...
      if resources.length > 1
        fail ArgumentError, 'invalid value for "resources", number of items must be less than or equal to 1.'
      end
